  ...
  return (
    <section className="relative overflow-hidden px-4 py-20 sm:px-6 sm:py-28 md:py-32">
      ...
      <div className="relative mx-auto w-full max-w-7xl">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.8 }} className="mb-8 flex flex-col gap-5 sm:mb-10 md:flex-row md:items-end md:justify-between">
          <div>
            <span className="text-[10px] uppercase tracking-[0.3em] text-white/40 sm:text-xs sm:tracking-[0.35em]">02 — Selected Works</span>
            <h2 className="mt-4 max-w-3xl text-3xl font-medium tracking-[-0.05em] text-white sm:text-4xl md:mt-5 md:text-6xl">Projects that taught me how to build.</h2>
          </div>
          <p className="max-w-xs text-xs leading-6 text-white/35 sm:text-sm">A selection of things I built while learning through practice, experimentation, and real problems.</p>
        </motion.div>
        ...
        <div className="relative overflow-hidden rounded-[1.5rem] border border-white/[0.09] bg-white/[0.018] p-2.5 shadow-[0_30px_90px_rgba(0,0,0,0.35)] backdrop-blur-2xl sm:rounded-[2rem] sm:p-4 md:p-5">
          ...
          <div className="relative flex h-[500px] items-center justify-center overflow-hidden rounded-[1.15rem] border border-white/[0.06] bg-black/20 [perspective:1200px] sm:h-[530px] sm:rounded-[1.5rem] md:h-[570px]">
            ...
            <div className="absolute bottom-4 left-1/2 z-40 flex -translate-x-1/2 items-center gap-1.5 rounded-full border border-white/[0.08] bg-black/45 px-2.5 py-2 backdrop-blur-xl sm:bottom-5 sm:gap-2 sm:px-3">{projects.map((project, index) => <button key={project.title} type="button" aria-label={`Go to ${project.title}`} onClick={() => selectProject(index)} className={`h-1.5 rounded-full transition-all duration-500 ${index === activeIndex ? "w-7 bg-white/80 sm:w-9" : "w-1.5 bg-white/15 hover:bg-white/35"}`} />)}</div>
          </div>
