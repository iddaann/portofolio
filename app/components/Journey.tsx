          ...
          <div className="relative">
            ...
            <div className="space-y-12 sm:space-y-16 md:space-y-24">
              {milestones.map((item, i) => {
                const left = i % 2 === 0;
                return <motion.article key={item.step} initial={{ opacity: 0, y: 35, filter: "blur(6px)" }} whileInView={{ opacity: 1, y: 0, filter: "blur(0px)" }} viewport={{ once: true, amount: 0.25 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="relative grid grid-cols-[30px_minmax(0,1fr)] gap-4 md:grid-cols-2 md:gap-16">
                  <div className="hidden md:block" />
                  <div className={`${left ? "md:col-start-1 md:row-start-1 md:text-right" : "md:col-start-2 md:row-start-1"} col-start-2 row-start-1 min-w-0 pl-0`}>
                    <span className="font-mono text-[8px] tracking-[0.25em] text-white/25 sm:text-[9px]">{item.step}</span>
                    <h3 className="mt-2 text-xl font-medium leading-tight tracking-[-0.035em] text-white sm:mt-3 sm:text-2xl md:text-3xl">{item.title}</h3>
                    <p className={`mt-3 max-w-md text-xs leading-6 text-white/40 sm:mt-4 sm:text-sm sm:leading-7 md:text-base ${left ? "md:ml-auto" : ""}`}>{item.description}</p>
                    <span className={`mt-4 inline-flex max-w-full rounded-full border border-white/[0.08] bg-white/[0.025] px-2.5 py-1.5 font-mono text-[7px] tracking-[0.15em] text-white/25 sm:mt-5 sm:px-3 sm:text-[8px] sm:tracking-[0.18em] ${left ? "md:mr-0" : ""}`}>{item.tag}</span>
                  </div>
