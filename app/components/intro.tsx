...
export default function Intro() {
  ...
  const reveal = { once: true, amount: 0.25 } as const;
    ...
    <section ref={ref} className="relative z-20 flex min-h-[92svh] items-center px-5 py-24 sm:px-6 sm:py-28 md:min-h-screen md:px-16 md:py-32 lg:px-24">
      <div className="mx-auto w-full max-w-6xl">
        <div className="grid gap-10 md:grid-cols-[48px_1fr] md:gap-8">
          ...
          <div>
            <motion.h2 style={{ y: headingY, scale: headingScale }} initial={{ opacity: 0, y: 60, filter: "blur(12px)" }} whileInView={{ opacity: 1, y: 0, filter: "blur(0px)" }} viewport={reveal} transition={{ duration: 1.05, ease }} className="max-w-4xl text-left text-[clamp(2.15rem,7vw,6rem)] font-medium leading-[0.98] tracking-[-0.055em] text-white">
              I learn by building
            ...
            <div className="mt-12 grid gap-8 sm:mt-16 md:grid-cols-[1fr_auto] md:items-end md:gap-12">
              <motion.div style={{ y: descY }} initial={{ opacity: 0, y: 35 }} whileInView={{ opacity: 1, y: 0 }} viewport={reveal} transition={{ duration: 0.9, delay: 0.15, ease }} className="max-w-xl">
            ...
            <motion.div initial={{ scaleX: 0, opacity: 0 }} whileInView={{ scaleX: 1, opacity: 1 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 1.1, delay: 0.35, ease }} className="mt-16 h-px w-24 origin-left bg-white/20 sm:mt-20" />
          </div>
        </div>
      </div>
    </section>
